...
@Component({
  ...
  templateUrl: '../constellation-viewer-shared-template.component.html',
...
export default class ConstellationViewerComponent {
  ...
  protected readonly colorScheme$ = inject(ColorSchemeObserver).observe();

  protected readonly constellations$ =
    this.constellationLoader.getConstellations();

  protected readonly selectedConstellation$ = toObservable(this.id).pipe(
    switchMap(iauAbbreviation => {
      ...
      if (iauAbbreviation) {
        ...
        return this.constellationLoader.getConstellation(iauAbbreviation);
      } else {
