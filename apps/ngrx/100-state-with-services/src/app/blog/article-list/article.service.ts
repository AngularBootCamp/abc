...
export class ArticleService {
  ...
  readonly articles: Observable<Article[]> = this._articles.asObservable();
  ...
  deleteArticle(article: Article) {
    const articles = this._articles.value.filter(p => p.id !== article.id);
    this._articles.next(articles);
  }

  createArticle(article: Omit<Article, 'id'>) {
    let articles = this._articles.value;
    const newId = Math.max(...this._articles.value.map(p => p.id)) + 1;
